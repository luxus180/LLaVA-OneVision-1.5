...
def save_checkpoint(queue, args):
    ...
    if args.target_tensor_parallel_size is None:
        if hasattr(md, 'previous_tensor_parallel_size'):
            ...
            print("loader did not provide a tensor parallel size and --target-tensor-parallel-size not provided "
                  "on command line. Default to 1.")
    ...
    if args.target_pipeline_parallel_size is None:
        if hasattr(md, 'previous_pipeline_parallel_size'):
            ...
            print("loader did not provide a pipeline parallel size and --target-pipeline-parallel-size not provided "
                  "on command line. Default to 1.")
    ...
    models = [[[None for _ in range(args.target_tensor_parallel_size)]
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        if pp_rank == args.target_pipeline_parallel_size - 1:
            ...
            pp_local_models = [get_local_model(pp_rank, ep_rank, tp_rank) 
                               for ep_rank in range(args.target_expert_parallel_size)
                               for tp_rank in range(args.target_tensor_parallel_size)]
