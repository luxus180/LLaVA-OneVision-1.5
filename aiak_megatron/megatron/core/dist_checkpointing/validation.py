...
def maybe_report_missing_and_unexpected_keys(
    ...
    unexpected_title_msg = (f'Unexpected keys (not found in the checkpoint) encountered in '
                            f'the provided sharded state dict. ')
...
def _validate_common_state_dict(common_state_dict: CommonStateDict) -> None:
    ...
    consistant across ranks before saving.
    ...
    if rank == 0:
        ...
        if len(common_state_dict_diff) != 0:
            logger.warning(
                f'There is difference in the common state dict in different ranks. '
                f'The differences are {common_state_dict_diff}'
            )
...
def _validate_sharding_for_key_flattened(tensors_by_shard):
    ...
    if starts[0] != 0 or stops[-1] != expected_size or not np.all(starts[1:] == stops[:-1]):
        raise CheckpointingException(
            f'Flattened ranges dont cover the whole shard {tensors_by_shard[0]} of size {expected_size}. '
            f'Ranges: {(starts, stops)}'
        )
