...
def load_indexed_datasets(project_dir: str, indexed_dataset_infos: List[Dict]) -> None:
    ...
    Args:
        project_dir (str): Path to Retro project dir.
        indexed_dataset_infos (List[Dict]): List of metadata for each dataset (see `save_indexed_dataset_infos()` for
            more details.
    """
...
def get_individual_chunk_db(project_dir: str, ds_id: int, ds_info: dict) -> np.ndarray:
    ...
    Returns:
        Array of chunk start/end indexes for this dataset, where the chunk indexes can be used for indexing into the
            corresponding indexed dataset.
    """
...
def get_merged_dataset(
    ...
    Args:
        ...
        indexed_dataset_infos (Optional[List[Dict]]): Optionally, pre-loaded list of dataset metadata
            (see `save_indexed_dataset_infos()` for more detail). If not provided, the indexed dataset infos will be
            loaded from disk.
...
def get_merged_datasets(project_dir: str, chunk_length: int, eod_token_id: int) -> dict:
    ...
    Returns:
        A dict mapping DB type ('sampled', 'train', or 'valid') to the corresponding DBDataset, which is a dataset that
            wraps the HDF5 chunk index array.
    """
    fns = {
