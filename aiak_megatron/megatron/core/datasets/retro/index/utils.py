...
def num_samples_to_block_ranges(
    ...
    Args:
        config (RetroPreprocessingConfig): Retro preprocessing config.
        num_samples (int): Split `num_samples` into consecutive block ranges, where each block is size
...
def get_training_data_root_dir(config: RetroPreprocessingConfig) -> str:
    ...
    Returns:
        Path to the training data directory, which contains both training embedding blocks and the final merged
            training embeddings.
    """
...
def get_training_data_block_dir(config: RetroPreprocessingConfig) -> str:
    ...
    Returns:
        Path to the directory containing the training embedding blocks, which will be later merged into a single
            embedding array.
    """
