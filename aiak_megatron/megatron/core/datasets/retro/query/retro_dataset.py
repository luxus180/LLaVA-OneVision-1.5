...
class RetroDataset(torch.utils.data.Dataset):
    ...
    ** Note: chunk dataset wraps original GPT dataset (see gpt_chunk_dataset.py).
    ...
    Args:
        ...
        chunk_dataset (GPTChunkDataset): GPT chunk dataset, which is a wrapper around a standard GPT dataset that
            breaks each sample into chunks.
    ...
    def __getitem__(self, sample_idx: int) -> dict:
        ...
        Returns:
            A dict consisting of GPT sample (attribute 'text') and corresponding neighbor chunk IDs ('neighbor_chunks',
                for indexing chunk database) and neighbor token IDs (corresponding chunk database GPT tokens).
        """
...
def get_retro_datasets(
    ...
    Args:
        ...
        gpt_datasets (dict): Mapping of data split key ('train', 'valid', or 'test') to the original sequence-length
