...
class _MMapBinReader(_BinReader):
    ...
    def read(self, dtype: Type[numpy.number], count: int, offset: int) -> numpy.ndarray:
        ...
        Returns:
            numpy.ndarray: An array with `count` items and data-type `dtype` constructed from reading
                bytes from the data file starting at `offset`.
        """
...
class _FileBinReader(_BinReader):
    ...
    def read(self, dtype: Type[numpy.number], count: int, offset: int) -> numpy.ndarray:
        ...
        Returns:
            numpy.ndarray: An array with `count` items and data-type `dtype` constructed from reading bytes from
                the data file starting at `offset`.
        """
...
class _S3BinReader(_BinReader):
    ...
    Args:
        ...
        bin_chunk_nbytes (int, optional): If not None, then maintain an in-memory cache to speed up calls to the
            `read` method. Furthermore, on a cache miss, download this number of bytes to refresh the cache.
            Otherwise (None), do not maintain an in-memory cache. A class that inherits from _BinReader may not
            implement caching in which case it should assert that `bin_chunk_nbytes` is None at initialization.
    """
...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    Args:
        ...
        s3_config (Optional[S3Config]): Supplied only for data stored on S3. IndexedDataset downloads the index
            (.idx) file to `s3_config.path_to_idx_cache` and streams data from the data (.bin) file in
            `s3_config.bin_chunk_nbytes` blocks. Note that `mmap` must be disabled for S3 data loading.
            Defaults to None.
    """
